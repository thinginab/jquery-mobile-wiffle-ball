...
break;var outcome=game.halfinnings[i].atbats[j].pitches[game.halfinnings[i].atbats[j].pitches.length-1];if(outcome.indexOf("-skip")===-1)
...
{var game=games[k];for(var i=0;i<game.halfinnings.length;i++)
{for(var j=0;j<game.halfinnings[i].atbats.length;j++)
{var outcome=game.halfinnings[i].atbats[j].pitches[game.halfinnings[i].atbats[j].pitches.length-1];if(outcome.indexOf("-skip")===-1)
{if(isbatter&&game.halfinnings[i].atbats[j].batter.name===player&&regex.test(outcome))
...
return outcomes;}
function Game(type){"use strict";this.team1=new Team();this.team2=new Team();this.id="";this.season="";this.innings=CONFIG.inningsPerGame;this.halfinnings=[];};function Base(type){"use strict";this.number=0;this.player=null;};function Team(type){"use strict";this.name="";this.players=[];this.currentbatter=-1;this.currentpitcher=-1;};function Player(type){"use strict";this.name="";this.skip=false;};function HalfInning(type){"use strict";this.teamFielding=null;this.teamBatting=null;this.number=1;this.top=true;this.currentouts=0;this.atbats=[];};function AtBat(type){"use strict";this.pitcher=null;this.pitcherIndex=0;this.batter=null;this.batterIndex=0;this.fielder=null;this.pitches=[];this.runsScored=0;this.runnersScored=[];this.runnersOut=[];this.currentstrikes=0;this.currentballs=0;this.currentfouls=0;this.bases=null;};var currentGame;var currentGameIndex;var currentSeasonIndex=-1;var games;var savedGames;var viewMode="wide";var undos=[];var redos=[];var clk=null;function processSizeChange()
{"use strict";if($("#game:visible").size()>0)
setGameViewMode();else if($("#home:visible").size()>0)
setHomeViewMode();}
$(function(){"use strict";if('ontouchstart'in document.documentElement)
...
games=getGames();CONFIG=getConfig();getSavedGames();if(CONFIG.isMobile)
...
setHomeViewMode();setGameViewMode();$("#home").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindHome,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});bindHome();$("#continue,#stats").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindGames,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});bindGames();$("#config").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindConfig,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});bindConfig();$("#newgame").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindNewGame,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});bindNewGame();var gameregex=/.*\#game(stats)?\&ui-page=game(\d*)(\&season(\d*))?/ig;var matches=gameregex.exec(window.location.href);if(matches!=null&&matches.length>1){showPageLoadingMsg();currentGameIndex=parseInt(matches[2]);if(matches.length>4&&matches[4]!=null)
currentSeasonIndex=parseInt(matches[4]);else
currentSeasonIndex=-1;setTimeout(loadCurrentGame,CONFIG.processDelay);}
var seasonregex=/.*\#seasonstats?\&ui-page=season(\-?\d*)/ig;matches=seasonregex.exec(window.location.href);if(matches!=null&&matches.length>1){showPageLoadingMsg();currentSeasonIndex=parseInt(matches[1]);setTimeout(bindSeasonStats,CONFIG.processDelay);}
$("#game").live("pageshow",function(){showPageLoadingMsg();setTimeout(loadCurrentGame,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});$("#gamestats").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindGameStats,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});$("#seasonstats").live("pageshow",function(){showPageLoadingMsg();setTimeout(bindSeasonStats,CONFIG.processDelay);}).live("pagebeforeshow",function(){showPageLoadingMsg($(this));});gameInit();newGameInit();configInit();});function substitutePlayer(team,playerIndex,newname)
...
processultimeout=setTimeout(function(){pitchulprocessing=true;processultimeout=null;var $li=$this.parents("li:first").removeClass("ui-btn-active");if(CONFIG.isMobile)
{processPitch($this.attr("data-pitch"),$this.attr("data-fielded-by"));}
if(clrultimeout!=null)
{clearTimeout(clrultimeout);clrultimeout=null;}
clrultimeout=setTimeout(function(){clrultimeout=null;$this.removeClass("ui-btn-active");pitchulprocessing=false;},CONFIG.clearDelay);pitchulprocessing=true;},CONFIG.processDelay);}});var pitchulprocessing2=false;var processultimeout2=null;var clrultimeout2=null;$("ul:not(.inplay) li a[data-pitch]").live(CONFIG.clickEvent,function(){if(!pitchulprocessing2)
...
$this.parent().addClass(".ui-btn-active");if(!CONFIG.isMobile)
{processPitch($this.attr("data-pitch"),$this.attr("data-fielded-by"));}
processultimeout2=setTimeout(function(){pitchulprocessing2=true;processultimeout2=null;var $li=$this.parents("li:first");if(CONFIG.isMobile)
{processPitch($this.attr("data-pitch"),$this.attr("data-fielded-by"));}
if(clrultimeout2!=null)
{clearTimeout(clrultimeout2);clrultimeout2=null;}
clrultimeout2=setTimeout(function(){clrultimeout2=null;$this.removeClass("ui-btn-active");pitchulprocessing2=false;},CONFIG.clearDelay);pitchulprocessing2=true;},CONFIG.processDelay);}});var pitchbtnprocessing=false;var clrbtntimeout=null;var processtimeout=null;$("#pitches a[data-pitch]").bind(CONFIG.clickEvent,function(){var $this=$(this);if(!pitchbtnprocessing)
...
pitchbtnprocessing=true;processtimeout=setTimeout(function(){pitchbtnprocessing=true;processtimeout=null;processPitch($this.attr("data-pitch"),$this.attr("data-fielded-by"));if(clrbtntimeout!=null)
{clearTimeout(clrbtntimeout);clrbtntimeout=null;}
clrbtntimeout=setTimeout(function(){clrbtntimeout=null;$this.removeClass("ui-btn-active");pitchbtnprocessing=false;},CONFIG.clearDelay);pitchbtnprocessing=true;},CONFIG.processDelay);}});$("#undo").bind("click",function(){var $this=$(this);if(!$this.hasClass(".ui-btn-active"))
$this.addClass(".ui-btn-active");undo();setTimeout(function(){var $li=$this.parents("li:first").removeClass("ui-btn-active");},CONFIG.clearDelay);});$("#basePath,#basePath_n").bind("click",function(){var hi=getCurrentHalfInning();if(hi.atbats.length>1)
...
$(".baserunner li.runner").draggable({revert:true,axis:"y",opacity:.7,cursorAt:{left:0,top:0}});if($(".baserunner").attr("data-inited")!=="true")
{$(".baserunner").droppable({tolerance:"touch",accept:function(draggable){return draggable.is("li.runner")&&($(this).has(draggable).length!==0||$(this).attr("id").indexOf("_")===-1||!$(this).is(":has(li)"));},activeClass:"baserunner-active",hoverClass:"baserunner-hover",drop:function(event,ui){$(this).prepend(ui.draggable);}});$(".baserunner").attr("data-inited","true");}
if($("#saverunners").attr("data-inited")!=="true")
{$("#saverunners").bind(CONFIG.clickEvent,function(){$("ul:not(#runnersOut) li.runner,ul#runnersOut li.runner").each(function(){var $r=$(this);var $p=$r.parent();if($p.attr("id")==="runnersout")
{if($r.attr("data-out")!=="true")
{if($r.attr("data-base")!=null)
{if(ab.bases[parseInt($r.attr("data-base"))].player.name==$r.text())
{lastab.runnersOut[lastab.runnersOut.length]=ab.bases[parseInt($r.attr("data-base"))].player;ab.bases[parseInt($r.attr("data-base"))].player=null;}}
if($r.attr("data-scored")==="true")
{lastab.runsScored--;var index=parseInt($r.attr("data-scorerindex"));lastab.runnersOut[lastab.runnersOut.length]=lastab.runnersScored[index];lastab.runnersScored.remove(index,index+1);}
runnerOut();}}
else if($p.attr("id")=="runnersscored")
{if($r.attr("data-scored")!=="true")
{lastab.runsScored++;if($r.attr("data-base")!=null)
{lastab.runnersScored[lastab.runnersScored.length]=ab.bases[parseInt($r.attr("data-base"))].player;ab.bases[parseInt($r.attr("data-base"))].player=null;}
if($r.attr("data-out")==="true")
{hi.currentouts--;var index=parseInt($r.attr("data-outindex"));lastab.runnersScored[lastab.runnersScored.length]=lastab.runnersOut[index];lastab.runnersOut.remove(index,index+1);}}}
else
{if($r.attr("data-base")!=null&&$r.attr("data-base")!=$p.attr("data-base"))
{ab.bases[parseInt($p.attr("data-base"))].player=ab.bases[parseInt($r.attr("data-base"))].player;ab.bases[parseInt($r.attr("data-base"))].player=null;}
if($r.attr("data-out")==="true")
{hi.currentouts--;var index=parseInt($r.attr("data-outindex"));ab.bases[parseInt($p.attr("data-base"))].player=lastab.runnersOut[index];lastab.runnersOut.remove(index,index+1);}
if($r.attr("data-scored")==="true")
{lastab.runsScored--;var index=parseInt($r.attr("data-scorerindex"));ab.bases[parseInt($p.attr("data-base"))].player=lastab.runnersScored[index];lastab.runnersScored.remove(index,index+1);}}});hi.atbats[hi.atbats.length-1]=ab;hi.atbats[hi.atbats.length-2]=lastab;currentGame.halfinnings[currentGame.halfinnings.length-1]=hi;$('.ui-dialog').dialog('close');});$("#saverunners").attr("data-inited","true");}
...
$this.addClass(".ui-btn-active");redo();setTimeout(function(){$this.parents("li:first").removeClass("ui-btn-active");},CONFIG.clearDelay);});$("table.pitcher").bind(CONFIG.clickEvent,function(){var $this=$(this);if(!$this.hasClass(".ui-btn-active"))
...
return;}
getCurrentTeamFielding().currentpitcher=newpitcherindex;var atbat=getCurrentAtBat();atbat.pitcherIndex=newpitcherindex;atbat.pitcher=getCurrentTeamFielding().players[newpitcherindex];saveGames(games);bindGame();$('.ui-dialog').dialog('close');});setTimeout(function(){var $li=$this.removeClass("ui-btn-active");},CONFIG.clearDelay);});$("table.batter").bind(CONFIG.clickEvent,function(){var $this=$(this);if(!$this.hasClass(".ui-btn-active"))
...
return;}
getCurrentTeamBatting().currentbatter=newbatterindex;var atbat=getCurrentAtBat();atbat.batterIndex=newbatterindex;atbat.batter=getCurrentTeamBatting().players[newbatterindex];saveGames(games);bindGame();$('.ui-dialog').dialog('close');});$("#changebatter").find(".ui-header").removeClass("ui-corner-top");setTimeout(function(){$this.parents("li:first").removeClass("ui-btn-active");},CONFIG.clearDelay);});$(document).delegate("#post",CONFIG.clickEvent,function(){var $this=$(this);if(!$this.parent().hasClass(".ui-btn-active"))
$this.parent().addClass(".ui-btn-active");if(currentGame.season&&currentGame.season.length>0)
{post();}
else
{$.mobile.changePage('#setseason','pop',false,true);$("#setseason").find(".ui-header").removeClass("ui-corner-top");$("#saveseason").bind(CONFIG.clickEvent,function(){currentGame.season=$('#seasonName').val();saveGames(games);$("#server").hide();$('.ui-dialog').dialog('close');});}
setTimeout(function(){var $li=$this.parents("li:first").removeClass("ui-btn-active");},CONFIG.clearDelay);});$("#output").bind(CONFIG.clickEvent,function(){var $this=$(this);if(!$this.parent().hasClass(".ui-btn-active"))
$this.parent().addClass(".ui-btn-active");output();setTimeout(function(){var $li=$this.parents("li:first").removeClass("ui-btn-active");},CONFIG.clearDelay);});}}
var configinited=false;function configInit(){if(!configinited)
...
{newgameinited=true;$("#newgameform").submit(function(e){currentGame=new Game();currentGame.innings=$("#innings").val();currentGame.team1.name=$("#team1").val();currentGame.team2.name=$("#team2").val();try
...
$("a[href='#stats']").show();setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}
...
var $basePath=$("#basePath");if($basePath[0].getContext)
{drawBases($basePath);drawBases($("#basePath_n"));}
if(viewMode==="wide")
...
{setTimeout(loadCurrentGame,100);return;}
...
{undos=[];undos[0]=JSON.stringify(currentGame);setupRules();}
bindGame();bindGameStats();setupFielders();setGameViewMode();}
...
return;if(!savedgamesloaded)
{setTimeout(bindNewGame,100);return;}
...
setupRules();undos=[];redos=[];undos[0]=JSON.stringify(currentGame);if($(this).is("[data-recreate=true]"))
...
nextAtBat();currentGameIndex=games.length;currentSeasonIndex=-1;games[currentGameIndex]=currentGame;undos=[];redos=[];undos[0]=JSON.stringify(currentGame);recreateGame();saveGames(games);$.mobile.changePage($(this).find("a").attr("href"),{changeHash:true,dataUrl:"#game&ui-page=game"+(games.length-1)});}
else
...
$("ul li.ui-btn-active").removeClass("ui-btn-active");isloading=false;setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}
...
function bindGame()
{"use strict";if(!savedgamesloaded)
{setTimeout(bindGame,100);return;}
if(currentGame==null)
...
{atbat.bases=[];for(var i=0;i<4;i++)
{atbat.bases[i]=new Base();atbat.bases[i].number=i+1;}}
var $basePath=$("#basePath");if($basePath[0].getContext)
{drawBases($basePath,atbat.bases);drawBases($("#basePath_n"),atbat.bases);}
else
...
$(".dp:not([data-disabled])").show();else
$(".dp:not([data-disabled])").hide();if((atbat.bases[0].player!=null||atbat.bases[1].player!=null||atbat.bases[2].player!=null)&&halfinning.currentouts<2)
$(".sac:not([data-disabled])").show();else
$(".sac:not([data-disabled])").hide();if((currentGame.season&&currentGame.season.length>0)||!window.navigator.onLine||!CONFIG.connectToServer)
$("#server").hide();else
$("#server").show();if($("#other").nextAll("li:visible").length===0)
$("#other").hide();else
$("#other").show();setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}
var draw_current_bases=null;function drawBases($canvas,bases)
{"use strict";if(bases==null)
bases=draw_current_bases;else
draw_current_bases=bases;if(bases==null)
return;$canvas.attr("width",1).attr("height",1);var basePathCw=$canvas.parent().width();var basePathCh=$canvas.parent().parent().height()-4;var minSide=Math.min(basePathCw,basePathCh);if(minSide>0)
{var diamondsize=minSide*(.7);var basesize=diamondsize/4.0;$canvas.attr("width",basePathCw).attr("height",basePathCh);var basePathC=$canvas[0].getContext('2d');basePathC.strokeStyle="#947333";basePathC.fillStyle="#D6522C";var translateX=(basePathCw-diamondsize)/2.0;var translateY=(basePathCh-diamondsize)/2.0;var tranCenterX=basePathCw/2.0;var tranCenterY=basePathCh/2.0;basePathC.translate(tranCenterX,tranCenterY);basePathC.rotate(45.0*Math.PI/180.0);basePathC.translate(-tranCenterX,-tranCenterY);basePathC.translate(translateX,translateY);basePathC.strokeRect(0,0,diamondsize,diamondsize);basePathC.strokeRect(diamondsize-basesize,0,basesize,basesize);if(bases[0].player!=null)
basePathC.fillRect(diamondsize-basesize,0,basesize,basesize);basePathC.strokeRect(0,0,basesize,basesize);if(bases[1].player!=null)
basePathC.fillRect(0,0,basesize,basesize);basePathC.strokeRect(0,diamondsize-basesize,basesize,basesize);if(bases[2].player!=null)
basePathC.fillRect(0,diamondsize-basesize,basesize,basesize);basePathC.strokeRect(diamondsize-basesize,diamondsize-basesize,basesize,basesize);}}
...
nextAtBat(newbases);}
saveGames(games);bindGame();isprocessing=false;}}
...
function runnerOut()
{"use strict";var halfinning=getCurrentHalfInning();halfinning.currentouts++;if(halfinning.currentouts>=CONFIG.outsPerInning)
{nextAtBat();}}
function nextAtBat(bases)
{"use strict";var halfinning=null;if(currentGame.halfinnings.length>0)
halfinning=getCurrentHalfInning();var inningchange=halfinning!=null&&halfinning.currentouts>=CONFIG.outsPerInning;if(currentGame.halfinnings.length===0||inningchange)
...
var setupFieldersRunning=false;function setupFielders()
{"use strict";if($("#game ul.inplay").is(":visible"))
{$("#game ul.inplay").hide();}
try
{$("#game ul.inplay").listview("refresh");}
catch(exc){setTimeout(setupFielders,100);return;}
if(!setupFieldersRunning)
{setupFieldersRunning=true;var fielded=$("li:has(a[data-fielded-by])");fielded.find("a[data-fielded-by]").attr("data-fielded-by","");fielded.find("span.fieldedby").remove();for(var i=fielded.length;i>=0;i--)
{var pitch=fielded.eq(i).find("a[data-fielded-by]").attr("data-pitch");if(pitch)
{var els=fielded.filter(":has([data-pitch="+pitch+"])").slice(1);i-=els.length;els.remove();fielded=$("li:has(a[data-fielded-by])");}}
var teamfielding=getCurrentTeamFielding();for(var i=0;i<fielded.length;i++)
{var item=fielded.eq(i);for(var j=1;j<teamfielding.players.length;j++)
{var newi=item.clone();newi.find("a[data-fielded-by]").attr("data-fielded-by",teamfielding.players[j].name).append("<span class='fieldedby'> By "+teamfielding.players[j].name+"</span>");item.after(newi);}
item.find("a[data-fielded-by]").attr("data-fielded-by",teamfielding.players[0].name).append("<span class='fieldedby'> By "+teamfielding.players[0].name+"</span>");}
...
outsInPlay=0;var atbat=getCurrentAtBat();var halfInning=getCurrentHalfInning();var newbases=atbat.bases.clone();for(var i=newbases.length-1;i>=0;i--)
{if(newbases[i].player!=null)
...
{"use strict";if(value!=null&&value.players!=null&&value.currentbatter!=null&&value.currentpitcher!=null&&value.name!=null&&value.getId==null)
{value.getId=function(){var id=this.name;for(var i=0;i<this.players.length;i++)
id+=this.players[i].name;return id;};}
if(value!=null&&value.players!=null&&value.currentbatter!=null&&value.currentpitcher!=null&&value.name!=null&&value.innings==null)
value.innings=CONFIG.inningsPerGame;if(value!=null&&value.pitcher!=null&&value.pitcherIndex!=null&&value.batter!=null&&value.batterIndex!=null&&value.runnersOut==null)
value.runnersOut=[];return value;}
function getGames()
{"use strict";if(localStorage["games"]==null)
return[];return JSON.parse(localStorage["games"],gameReviver);}
function getConfig()
...
config.halfInningSkip=true;return config;}
var savedgamesloaded=true;function getSavedGames(){"use strict";if(CONFIG.connectToServer&&window.navigator.onLine)
{savedgamesloaded=false;$.getJSON(LOAD_URL,function(data){savedGames=JSON.parse(JSON.stringify(data),gameReviver);savedgamesloaded=true;setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}).error(function(jqXHR,textStatus,errorThrown){savedGames=[];savedgamesloaded=true;setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);});}}
function saveGames(games)
{"use strict";if(savedgamesloaded)
post();undos[undos.length]=JSON.stringify(currentGame);localStorage["games"]=JSON.stringify(games);}
function saveConfig()
{"use strict";localStorage["config"]=JSON.stringify(CONFIG);}
function undo()
{"use strict";if(undos.length-2>=0)
{redos[redos.length]=JSON.stringify(currentGame);currentGame=JSON.parse(undos[undos.length-2],gameReviver);games[currentGameIndex]=currentGame;saveGames(games);undos.remove(undos.length-2,undos.length-1);bindGame();}}
function redo()
{"use strict";if(redos.length>0)
{currentGame=JSON.parse(redos[redos.length-1],gameReviver);games[currentGameIndex]=currentGame;saveGames(games);redos.remove(redos.length-1);bindGame();}}
function post()
...
return;if(!savedgamesloaded)
{setTimeout(bindGameStats,100);return;}
setBoxScores($("#boxscore"));setTeamStats($("#team1batting"),currentGame.team1,true);setTeamStats($("#team2batting"),currentGame.team2,true);setTeamStats($("#team1pitching"),currentGame.team1,false);setTeamStats($("#team2pitching"),currentGame.team2,false);$("#team1batting").tablesorter();$("#team2batting").tablesorter();$("#team1pitching").tablesorter();$("#team2pitching").tablesorter();var emaillink="mailto:?subject="+escape("Game Stats For "+currentGame.team2.name+" at "+currentGame.team1.name)+"&body=";emaillink+=escape("<h2>Box Score</h2>"+getOuterHTML($("#boxscore")));emaillink+=escape("<h2>"+currentGame.team1.name+" Batting</h2>"+getOuterHTML($("#team1batting")));emaillink+=escape("<h2>"+currentGame.team2.name+" Batting</h2>"+getOuterHTML($("#team2batting")));emaillink+=escape("<h2>"+currentGame.team1.name+" Pitching</h2>"+getOuterHTML($("#team1pitching")));emaillink+=escape("<h2>"+currentGame.team2.name+" Pitching</h2>"+getOuterHTML($("#team2pitching")));$("#gamestatemail").attr("href",emaillink);setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}
...
{"use strict";if(!savedgamesloaded)
{setTimeout(bindSeasonStats,100);return;}
var games=getGames();CONFIG=getConfig();if(currentSeasonIndex>=0)
games=savedGames[currentSeasonIndex].games;bindSeasonStat($("#playerbatting"),games,true);bindSeasonStat($("#playerpitching"),games,false);$("#playerbatting").tablesorter();$("#playerpitching").tablesorter();var emaillink="mailto:?subject="+escape("Player Stats")+"&body=";emaillink+=escape("<h2>Batting</h2>"+getOuterHTML($("#playerbatting")));emaillink+=escape("<h2>Pitching</h2>"+getOuterHTML($("#playerpitching")));$("#seasonstatemail").attr("href",emaillink);setTimeout(function(){hidePageLoadingMsg();},CONFIG.clearDelay);}
